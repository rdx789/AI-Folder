...
- **Write at least 5 sample user prompts**, one per file — each a realistic
  personal-finance question to test the LLM against, answerable using the
  `data/` dataset via the agent's tools. Don't ship fewer. For example: "how
  much did I spend on dining out last month?", "what are my recurring
  subscriptions?", "which account had the most transactions?", "am I
  spending more on groceries than last quarter?", "find any unusual large
...
- Also include the **system prompt** here, in its own file, framing the model as
  a personal finance analyst assistant that answers questions about the
  user's accounts and transactions using its tools rather than guessing or
  inventing numbers.
